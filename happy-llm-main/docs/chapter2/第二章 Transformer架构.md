...
# 假设输入维度
Q: (batch_size, seq_len_q, d_k) = (2, 5, 8)
K: (batch_size, seq_len_k, d_k) = (2, 5, 8)

# 如果直接相乘 Q × K (不转置)
Q × K = (2, 5, 8) × (2, 5, 8) = 无法计算！

# 因为矩阵乘法要求第一个矩阵的列数 = 第二个矩阵的行数
# 转置后 K^T
K^T: (batch_size, d_k, seq_len_k) = (2, 8, 5)
# 现在可以计算 Q × K^T

Q × K^T = (2, 5, 8) × (2, 8, 5) = (2, 5, 5)
```
